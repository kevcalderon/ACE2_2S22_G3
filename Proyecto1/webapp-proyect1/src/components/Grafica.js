...
function Grafica(props) {
      ...
      if(datosExperimento){
        const data = {
          ...
          datasets:[
            {
              label: "Fuerza Golpe",
              data: datosExperimento.map((data) => data.fuerzaGolpe),
              borderColor: ['#FFFFFF'],
              backgroundColor: ['#f83e0e'],
              //fontColor: ['#ff2323']
            }   
    ...
    if(datosExperimento){
      const data = {
        ...
        datasets:[
          {
            label: "Velocidad",
            data: datosExperimento.map((data) => data.velocidad),
            borderColor: ['#FFFFFF'],
            backgroundColor: ['#4df2cf']
          }           
        ]
      };
  ...
  const Grafica3 = (datosExperimento) => {    
    if(datosExperimento){
      const data = {
        ...
        datasets:[
          {
            label: "Porcentaje Ritmo",
            data: datosExperimento.map((data) => data.porcentajeRitmo),
            borderColor: ['#FFFFFF'],
            backgroundColor: ['#edf550']
          }           
        ]
      };
      ...
      const options = {
        ...
        plugins: {          
            title: {
                display: true,
                text: 'Porcentaje de tiempo que se mantiene en la zona de ritmo',
                color:['#FFFFFF']
            },
        },
    };
  ...
  const url= `https://api-ace2-p1.herokuapp.com/entreno-dashboard/grafica/fechaInicial/${props.fechaInicio}/fechaFinal/${props.fechaFin}/idUsuario/${props.idUsuario}`;
